...
const formatAssistantMessage = (response: any): ChatMessage[] => {
  ...
  // Extraction correcte du message de l'assistant
  const mainMessage =
...
export function useClientDelivery(options?: UseClientDeliveryOptions): UseClientDeliveryResult {
  ...
  const sendChatMessage = useCallback(async (message: string) => {
    try {
      ...
      // Déterminer userId
      const userId = client?.ID_Client || 'defaultUserId';
      ...
      const data = await response.json();
      console.log('📄 Structure de la réponse du backend:', JSON.stringify(data, null, 2));
  
      if (data.success) {
        // Formater et ajouter les messages de l'assistant
        const assistantMessages = formatAssistantMessage(data);
        console.log('🛠 Messages formatés pour chatMessages:', assistantMessages);
      ...
      // Vérifier si err est une instance de Error
      let errorMessage = "Erreur lors de l'envoi du message";
      if (err instanceof Error) {
        errorMessage = err.message;
      }
  
      handleAPIError(err, 'chatError', errorMessage);
    } finally {
      setIsLoading(false);
    }
  }, [client, clearError, handleAPIError, maxChatMessages]);
